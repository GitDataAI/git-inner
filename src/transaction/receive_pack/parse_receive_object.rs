...
use crate::transaction::receive_pack::report_status::ReportStatusKind;
use crate::transaction::receive_pack::zlib_decode::decompress_object_data;
use crate::transaction::receive_pack::{ReceivePackRequest, ReceivePackRequestInfo};
...
impl Transaction {
    pub async fn parse_receive_object(
        ...
        while pack_count < request_info.pack_size {
            ...
            match object_type {
                ObjectType::Commit | ObjectType::Tree | ObjectType::Blob | ObjectType::Tag => {
                    ...
                    let hash = self.process_object_data(object_type, &obj_bytes, txn.clone()).await?;
                ...
                ObjectType::RefDelta => {
                    let hash_len = self.repository.hash_version.len();
                ...
                ObjectType::Unknown => {
                    self.report_status(ReportStatusKind::UnpackError("unknown type".to_string()), receive_pack_request.capabilities.clone())
                        .await;
                }
            }
            pack_count += 1;
        ...
        loop {
            ...
            for (obj_start, (base_hash, delta_bytes)) in unresolved.iter() {
                if resolved_ofs.values().any(|(h, _, _)| h == base_hash) {
                    let (full_bytes, obj) = RefDelta::apply_delta(base_hash, delta_bytes, txn.clone(), &resolved_ofs).await?;
                    let id = self.process_object_data(obj, &full_bytes, txn.clone()).await?;
                    resolved_ofs.insert(*obj_start, (id, full_bytes, obj));
            ...
            self.report_status(
                ReportStatusKind::Info(format!("Apply ref delta {:.1}%\r", progress)),
                receive_pack_request.capabilities.clone(),
            ).await;
            if resolved_count == 0 {
                break;
            }
        }
        self.report_status(ReportStatusKind::UnpackOk,receive_pack_request.capabilities.clone()).await;
        txn.commit().await?;
        for idx in receive_pack_request.ref_upload {
            if idx.is_create() {
                if self.repository.refs.create_refs(idx.ref_name.clone(), idx.new).await.is_ok() {
                    self.report_status(ReportStatusKind::Ok(idx.ref_name), receive_pack_request.capabilities.clone()).await;
                }
            } else if idx.is_update() {
                if self.repository.refs.update_refs(idx.ref_name.clone(), idx.new).await.is_ok() {
                    self.report_status(ReportStatusKind::Ok(idx.ref_name), receive_pack_request.capabilities.clone()).await;
                }
            }
        }
        self.report_status(ReportStatusKind::Done, receive_pack_request.capabilities.clone()).await;
        Ok(())
    }
}
